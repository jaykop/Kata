...
void UKataGraphForceDirectedLayoutStrategy::Layout(UEdGraph* _EdGraph)
{
...
FBox2D UKataGraphForceDirectedLayoutStrategy::LayoutOneTree(UKataGraphNodeBase* RootNode, const FBox2D& PreTreeBound)
{
	...
	for (int32 IterrationNum = 0; IterrationNum < MaxIteration; ++IterrationNum)
	{
		...
		while (CurrLevelNodes.Num() != 0)
		{
			for (int32 i = 0; i < CurrLevelNodes.Num(); ++i)
			{
				...
				UKataEdNode* EdNode_ParentNode = EdGraph->NodeMap[Node];

				for (int32 j = 0; j < Node->ChildrenNodes.Num(); ++j)
				{
					NextLevelNodes.Add(Node->ChildrenNodes[j]);

					UKataEdNode* EdNode_ChildNode = EdGraph->NodeMap[Node->ChildrenNodes[j]];
