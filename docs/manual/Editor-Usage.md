...
- 노드를 고르고 Create Comment를 실행하면 고른 노드를 감싸는 설명 영역이 생긴다.
  아무것도 고르지 않았을 때만 커서 자리에 기본 크기로 만든다.
- Ctrl+F를 누르면 Find in Graph 탭이 열린다. 노드 제목과 주석뿐 아니라 액션 에셋의 이름과 경로,
  전이의 Trigger Event Tag와 Required Action Window Tag로도 찾는다. 결과를 누르면 그 노드로 화면이 옮겨 간다.
  전이에 제목을 직접 지어 두어도 태그로 찾을 수 있다.
...
Conduit과 Alias는 액션을 실행하지 않는다. 전이를 따질 때만 지나가는 자리이므로 여기에 머무르지 않는다.

- **Conduit**은 길 가운데 두는 갈아타는 자리다. 출발지 여러 곳에서 목적지 여러 곳으로 가는 전이를 모았다가
  다시 퍼뜨려 엣지 수를 곱셈에서 덧셈으로 줄이고, 공통으로 요구하는 조건을 Entry Condition 한 곳에 적는다.
  Conduit에서 나가는 엣지는 떠나는 액션이 없으므로 Required Action Window Tag와 Timing을 보지 않는다.
  트리거는 들어온 엣지에서 이미 받았으니 비워 두거나 같은 트리거를 적는다.
- **Alias**는 출발지 여러 곳을 하나로 묶는 이름표다. 같은 목적지로 가는 전이를 노드마다 긋지 않고 한 번만 긋는다.
  Source Nodes에 그래프의 노드가 나열되므로 포함할 노드를 체크한다. Any State를 켜면 목록 대신 머무를 수 있는
  모든 노드를 포함한다. 사망이나 스턴처럼 무엇을 하고 있든 가로채야 하는 전이에 쓴다.
  Alias에서 나가는 엣지는 현재 액션을 떠나는 전이이므로 Window와 Timing을 그대로 적용한다. 이 점이 Conduit과 반대다.
  해석 결과가 지금 실행 중인 노드와 같으면 그 전이는 건너뛴다. 노드에 직접 그은 자기 엣지는 그대로 동작한다.

Conduit에서 나가는 엣지가 없거나 조건이 모두 막히면 그 전이는 성립하지 않는다. 경고가 따로 뜨지 않으므로
전이가 안 될 때 확인할 곳이다.
