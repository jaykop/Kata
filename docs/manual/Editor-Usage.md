...
갱신: 2026-09-24 · Play Montage 포즈 탐색 소스 구현, 이번 변경의 빌드·UI 실행 미검증
...
- Current Time은 재생 헤드의 현재 시각이다. 값을 직접 입력하거나 시간 눈금 및 태스크 클립이 없는 빈 영역을
  ...
  Play Montage가 해당 시각에 있으면 몽타주 에셋 포즈를 직접 표시한다. 시간 탐색 영역은 십자 커서,
  태스크 양쪽 끝은 좌우 크기 조절 커서를 쓴다.
- 포즈 탐색은 Start Section 시작 지점(비어 있으면 0)에서 태스크 경과 시간 × Play Rate × Montage RateScale만큼
  이동한 에셋 위치를 사용한다. 섹션 연결이 끊기거나 반복되어도 해당 에셋 위치의 포즈를 볼 수 있다.
  태스크 시작부터 끝까지 표시하며 몽타주 길이를 넘으면 마지막 포즈를 표시한다. 여러 Play Montage가 겹치면
  가장 늦게 시작한 항목, 시작 시각도 같으면 해석된 목록의 마지막 항목을 표시한다.
  태스크 구간 밖에서는 원래 메시 애니메이션 설정으로 돌아간다. 순간·Single Frame 태스크는 시작 시각에만 표시한다.
- 포즈 탐색은 Start Section부터 해당 위치까지의 루트 모션 이동·회전을 캐릭터에 반영한다. 메시와 캡슐이 함께 움직인다.
  앞뒤로 이동해도 같은 시각에서는 같은 배치를 사용하며, 태스크 구간 밖이나 Stop에서는 원래 배치로 돌아간다.
  탐색 중에는 월드가 진행하지 않으며 Notify·게임플레이 이벤트·충돌·중력을 재현하지 않는다.
- 일반 재생을 Pause한 뒤 Play를 누르면 남은 구간을 이어 재생한다. Current Time의 같은 값을 다시
  확정하는 동작은 탐색으로 처리하지 않는다.
- 시간 바나 입력값을 옮겨 포즈 탐색한 뒤 Play를 누르면 그 시각부터 재생한다. 액션을 처음부터 보이지 않게
  한 프레임 안에서 빠르게 실행한 뒤 이어 재생하므로, 루트 모션 위치·캡슐·다른 태스크의 효과가 실제 실행과 같다.
  먼 시각에서 누르면 잠깐 멈칫할 수 있다.
