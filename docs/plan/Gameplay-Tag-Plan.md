...
| 태그 소유 | 확정 | 태그는 프로젝트에만 둔다. 루트 이름은 `KataTag`다. Kata 플러그인은 태그를 정의하지 않고, `Config/Tags`와 생성 코드도 갖지 않는다. 플러그인 API와 에셋 프로퍼티는 임의의 `FGameplayTag`를 받으며 `KataTag`를 참조하지 않는다. 플러그인이 프로젝트 모듈을 참조하면 의존 방향이 역전되기 때문이다. 2026-09-24 플러그인 분리 뒤 사용자가 재확인했다. 태그 정의는 게임별 데이터라 "프로젝트는 샘플 전용" 원칙과 충돌하지 않는다([#1](https://github.com/jaykop/Kata/issues/1)). |
