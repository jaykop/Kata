...
Plugins/
  Kata/             코어              의존: GameplayAbilities
    ...
    KataRuntime     액션, 실행, GAS 연결, 외부 의존 없는 태스크, PreTask·PostTask
    KataGraph       그래프, 콤보 전이
...
3. 의존은 위성 → 코어 한 방향이다. 코어는 위성을 알지 못하며 확장 지점(PreTask·PostTask, 인터페이스, 델리게이트)만 제공한다.
...
| PM-2 코어 확장 지점 | 확정 | 2026-09-24 사용자 결정. 아래 "PM-2 설계"의 결정 1~7. 앞서 제안한 수명 주기 객체형 "액션 훅"은 채택하지 않고 PreTask·PostTask로 대체했다 |
...
| PM-2 | 높음 | 코어 확장 지점: PreTask·PostTask, 태스크의 Context 수정, 그래프 Context 다시 기록, 엣지 대상 유지 토글 | PM-0 | 위성 없이 코어만으로 빌드되고 Pre·PostTask가 없는 기존 액션·그래프 동작이 유지된다 |
...
`UKataAction`에 PreTask 목록과 PostTask 목록을 추가한다. 둘 다 기존 `UKataTask`·`UKataTaskInstance` 구조를 쓰며
서로 참조하거나 짝을 이루지 않는다. 부모 액션 상속과 자원 정리는 기존 태스크 규칙을 따른다.

- PreTask: 시작 조건을 통과해 액션이 시작될 때 타임라인 태스크보다 먼저 실행한다.
- PostTask: 타임라인이 끝나거나 액션이 어떤 사유로든 종료될 때 실행한다.

### B. 태스크의 Context 수정과 그래프 Context 다시 기록

현재 `UKataActionInstance`의 Context는 읽기만 가능하고, `UKataGraphInstance`는 시작 때 받은 Context를 모든 노드에 넘긴다.
PreTask가 액션 인스턴스의 Context를 수정할 수 있게 하고, 그래프는 수정된 `TargetActor`를 그래프 Context에 다시 기록해
다음 노드가 이어받게 한다.
...
| # | 항목 | 결정 |
|---|---|---|
| 1 | PreTask 실행 길이 | 같은 프레임에 끝나는 즉시 실행만 허용한다 |
| 2 | PostTask 실행 조건 | 모든 종료 사유에서 실행하되 태스크별 종료 사유 필터를 둔다. 기본값은 모든 사유다 |
| 3 | 편집 UI | 먼저 Details 목록으로 제공한다. 타임라인 트랙 표시는 필요할 때 검토한다 |
| 4 | Context 수정 권한 | PreTask만 수정할 수 있다. 타임라인 도중 대상이 바뀌면 실행 중인 태스크와 어긋나기 때문이다 |
| 5 | 다시 기록하는 시점 | PreTask 실행 직후. 액션 도중 전이가 일어나도 반영된다 |
...
- 병행 작업: PM-2는 `KataAction`, `KataResolvedAction`, `KataActionInstance`, `KataRuntimeTypes`, `KataEdge`, `KataGraphInstance`와
  에디터 Details를 수정한다. [#2](https://github.com/jaykop/Kata/issues/2) T05(프리뷰 무기 부착)도 `KataAction.h`를 수정하므로 PM-2의 해당 변경이 커밋된 뒤 진행한다.
- 공개 API: 위성 플러그인이 쓰는 코어 타입(`FKataContext`, 액션 인스턴스 이벤트, PreTask·PostTask 확장에 필요한 타입)은 모듈 API로 공개해야 한다.
- 에셋 호환: Pre·PostTask 목록과 엣지 토글은 새 프로퍼티다. 기존 에셋은 빈 목록과 토글 기본값 true로 읽혀 지금과 같게 동작해야 한다.
- 싱글플레이 전용 원칙은 모든 플러그인에 그대로 적용한다.

## 사용자 확인 항목

- PM-2 이후 기존 액션·그래프 에셋이 열리고 전과 같게 재생되는지, Pre·PostTask와 엣지 토글이 Details에 보이는지 확인한다.
