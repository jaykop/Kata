...
[계획](../plan/Gameplay-Tag-Plan.md)의 T01~T02를 구현했다. `Config/Tags/Native/*.ini`로부터 매 빌드마다
`Source/ProjectKata/KataTags.h/.cpp`를 생성하며, 사용자 확인을 마쳤다. 컴포넌트 태그 함수(T03) 이후는 보류했다.
...
- `UKataFL_Component`, Distance 조건 `ComponentTag` 전환, 프리뷰 무기 부착(T03~T05)은 다른 세션의 플러그인·모듈 정리 작업이 끝날 때까지 보류한다.
- 생성 스크립트는 Win64 전용이다.
...
| [게임플레이 태그 계획](../plan/Gameplay-Tag-Plan.md) | T01·T02 확인 완료, T03~T05 보류 |
