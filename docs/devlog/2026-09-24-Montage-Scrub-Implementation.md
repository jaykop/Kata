...
> 2026-09-25 안내: 본문은 직접 포즈 평가부터 실행 시뮬레이션으로 바뀐 순서의 이력이다.
> 현재 UAnimPreviewInstance 직접 평가 경로는 제거됐다. [최종 시뮬레이션 기록](2026-09-24-Preview-Scrub-Simulation.md)과
> [현재 설명서](../manual/Editor-Usage.md#프리뷰)를 먼저 읽는다. 당시 구현 완료를 모든 프리뷰 동작의 확인 완료로 보지 않는다.
...
| 수정 계획(완료 후 삭제) | 구현 완료와 실행 확인 대기를 구분. 스크럽 뒤 Play 정책 변경 결정. 탐색 방식 변경 결정 |
| 다음 작업 계획(2026-09-24 삭제, 이슈로 대체) | 구현 기록 연결 |
