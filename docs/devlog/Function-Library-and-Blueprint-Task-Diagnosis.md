...
> 2026-09-25 정리: 아래는 당시 진단과 결정이다. 현재 함수 사용법은 [Conditions](../manual/Conditions.md),
> BP·C++ 제작 절차는 [Task-Authoring](../manual/Task-Authoring.md)을 따른다. 현재 재초기화 함수명은 ResetForExecution이다.
...
- **루프 초기화:** 루프마다 인스턴스를 새로 만들지 않고 재사용한다. 현재 `ResetForExecution()`은 기본 실행 상태만 초기화하므로 사용자 BP 변수는 시작 시 초기화해야 한다. 옛 기록의 ResetForLoop 표기를 현행 이름으로 바로잡았다.
