...
2026-09-25 문서 대조에서 다음 현행 조건을 명확히 했다. 이번에는 소스를 수정하거나 UI로 재현하지 않았다.

- Play는 Instance가 Running일 때만 이어가고 그 밖에는 Start로 장면을 다시 만든다.
- 탐색 완료가 Repeat·자동 Reset을 발생시키지는 않지만 종료된 Instance는 다음 TickSimulation에서 대기 World Tick을 받는다.
  따라서 종료 뒤 Idle·중력까지 정지된 장면을 보장한다는 뜻은 아니다.
- 사용자에게서 루트 모션 미이동·Pause 후 재시작 보고가 있다. 정확한 재현 환경과 해당 증상의 해소 여부를
  이번 문서 작업에서 확인하지 않았으며, 과거의 포괄적인 동작 확인 보고만으로 해결됐다고 표시하지 않는다.

현재 조작·문제 해결은 [Editor-Usage](../manual/Editor-Usage.md#프리뷰)에 반영했다.
