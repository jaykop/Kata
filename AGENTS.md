...
- 문서 작성·수정 전에 해당 카테고리의 템플릿을 읽고 적용한다.
  ...
  - 후속 작업과 미확정 설계: `docs/plan/_Template.md`.
- 템플릿은 해당 폴더에 둔다. 새 문서는 템플릿을 복사하고 안내문·자리표시를 제거한다.
  기존 문서는 수정하는 범위부터 필요한 항목을 적용하며, 형식을 맞추기 위해 전체를 다시 쓰지 않는다.
- `Implementation-Status.md`는 현재 상태 요약, `Next-Work-Plan.md`는 후속 작업의 진입점이다.
  이 두 문서는 기존 주제별 구조를 유지할 수 있지만 갱신일, 근거, 확인 범위, 제한과 관련 링크를 관리한다.
- devlog에는 무엇을 바꿨고 왜 결정했는지, manual에는 현재 어떻게 사용하는지, plan에는 무엇이 남았고
  어떤 결정이 필요한지 기록한다. 같은 상세 설명을 여러 문서에 복사하지 말고 원문에 링크한다.
- API·UI·설정·실행 계약·기존 에셋 처리 방법이 바뀌면 관련 manual을 같은 작업에서 갱신한다.
  관련 plan의 항목도 구현 완료·보류·폐기 등 실제 상태로 바꾸고 결과 기록을 연결한다.
  상위 계획과 세부 계획이 함께 있으면 양쪽 상태를 맞춘다. 구현 완료와 실행 확인 완료는 구분한다.
